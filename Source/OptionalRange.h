...
inline OptionalRange readPlaybackRangeIntoBuffer (juce::Range<double>  loopRangeInTimeline,
                                                  const juce::ARAPlaybackRegion* playbackRegion,
                                                  juce::AudioBuffer<float>& buffer,
                                                  const std::function<AudioFormatReader* (ARA::PlugIn::AudioSource*)>& getReader)
{
	
	const auto loopStartInTimeline = loopRangeInTimeline.getStart();
	const auto regionStartInAudioMod = playbackRegion->getStartInAudioModificationTime();
	
    const auto loopRangeInAudioMod = loopRangeInTimeline.movedToStartAt ( loopStartInTimeline - regionStartInAudioMod);
    ...
    const Range<int64_t> loopRangeInAudioModSamples {
        ARA::roundSamplePosition (loopRangeInAudioMod.getStart() * audioModificationSampleRate),
        ARA::roundSamplePosition (loopRangeInAudioMod.getEnd() * audioModificationSampleRate) - 1
    };

    const auto inputOffset = jlimit ((int64_t) 0, audioSource->getSampleCount(), loopRangeInAudioModSamples.getStart());

    const auto outputOffset = -std::min (loopRangeInAudioModSamples.getStart(), (int64_t) 0);
    ...
    */
    const auto readLength = [&]
    {
        const auto sourceReadLength =
            std::min (loopRangeInAudioModSamples.getEnd(), audioSource->getSampleCount()) - inputOffset;
